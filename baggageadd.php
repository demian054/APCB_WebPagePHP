...
$baggage_add = NULL; // Initialize page object first

class cbaggage_add extends cbaggage {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["BAGGAGE_ID"] != "") {
				...
				$this->setKey("BAGGAGE_ID", $this->BAGGAGE_ID->CurrentValue); // Set up key
			} else {
				$this->setKey("BAGGAGE_ID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "baggageview.php")
	...
	function LoadDefaultValues() {
		$this->BOARDING_ID->CurrentValue = NULL;
		$this->BOARDING_ID->OldValue = $this->BOARDING_ID->CurrentValue;
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($this->getKey("BAGGAGE_ID")) <> "")
			$this->BAGGAGE_ID->CurrentValue = $this->getKey("BAGGAGE_ID"); // BAGGAGE_ID
		else
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// BOARDING_ID
		$this->BOARDING_ID->SetDbValueDef($rsnew, $this->BOARDING_ID->CurrentValue, 0, FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->BAGGAGE_ID->setDbValue($conn->Insert_ID());
				$rsnew['BAGGAGE_ID'] = $this->BAGGAGE_ID->DbValue;
			}
		} else {
...
// Create page object
if (!isset($baggage_add)) $baggage_add = new cbaggage_add();

// Page init
$baggage_add->Page_Init();

// Page main
$baggage_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$baggage_add->Page_Render();
?>
...
var CurrentForm = fbaggageadd = new ew_Form("fbaggageadd", "add");

// Validate form
fbaggageadd.Validate = function() {
...
fbaggageadd.Form_CustomValidate = 
...
fbaggageadd.ValidateRequired = true;
<?php } else { ?>
fbaggageadd.ValidateRequired = false; 
<?php } ?>
...
<?php $baggage_add->ShowPageHeader(); ?>
<?php
$baggage_add->ShowMessage();
?>
<form name="fbaggageadd" id="fbaggageadd" class="<?php echo $baggage_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($baggage_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $baggage_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $baggage_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
