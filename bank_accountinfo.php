...
//
class cbank_account extends cTable {
	var $BANK_ACCOUNT_ID;
	var $CODE;
	var $DESCRIPTION;
	var $ACCOUNT_NUMBER;
	...
	function __construct() {
		...
		// BANK_ACCOUNT_ID
		$this->BANK_ACCOUNT_ID = new cField('bank_account', 'bank_account', 'x_BANK_ACCOUNT_ID', 'BANK_ACCOUNT_ID', '`BANK_ACCOUNT_ID`', '`BANK_ACCOUNT_ID`', 3, -1, FALSE, '`BANK_ACCOUNT_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->BANK_ACCOUNT_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['BANK_ACCOUNT_ID'] = &$this->BANK_ACCOUNT_ID;

		// CODE
		$this->CODE = new cField('bank_account', 'bank_account', 'x_CODE', 'CODE', '`CODE`', '`CODE`', 200, -1, FALSE, '`CODE`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['CODE'] = &$this->CODE;

		// DESCRIPTION
		$this->DESCRIPTION = new cField('bank_account', 'bank_account', 'x_DESCRIPTION', 'DESCRIPTION', '`DESCRIPTION`', '`DESCRIPTION`', 200, -1, FALSE, '`DESCRIPTION`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['DESCRIPTION'] = &$this->DESCRIPTION;

		// ACCOUNT_NUMBER
		$this->ACCOUNT_NUMBER = new cField('bank_account', 'bank_account', 'x_ACCOUNT_NUMBER', 'ACCOUNT_NUMBER', '`ACCOUNT_NUMBER`', '`ACCOUNT_NUMBER`', 131, -1, FALSE, '`ACCOUNT_NUMBER`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ACCOUNT_NUMBER->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['ACCOUNT_NUMBER'] = &$this->ACCOUNT_NUMBER;

		// BANK_ID
		$this->BANK_ID = new cField('bank_account', 'bank_account', 'x_BANK_ID', 'BANK_ID', '`BANK_ID`', '`BANK_ID`', 3, -1, FALSE, '`BANK_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->BANK_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['BANK_ID'] = &$this->BANK_ID;

		// ACTIVE
		$this->ACTIVE = new cField('bank_account', 'bank_account', 'x_ACTIVE', 'ACTIVE', '`ACTIVE`', '`ACTIVE`', 16, -1, FALSE, '`ACTIVE`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`bank_account`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('BANK_ACCOUNT_ID', $rs))
				ew_AddFilter($where, ew_QuotedName('BANK_ACCOUNT_ID', $this->DBID) . '=' . ew_QuotedValue($rs['BANK_ACCOUNT_ID'], $this->BANK_ACCOUNT_ID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->BANK_ACCOUNT_ID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@BANK_ACCOUNT_ID@", ew_AdjustSql($this->BANK_ACCOUNT_ID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("bank_accountview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("bank_accountview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "bank_accountadd.php?" . $this->UrlParm($parm);
		else
			$url = "bank_accountadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("bank_accountedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("bank_accountadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("bank_accountdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "BANK_ACCOUNT_ID:" . ew_VarToJson($this->BANK_ACCOUNT_ID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->BANK_ACCOUNT_ID->CurrentValue)) {
			$sUrl .= "BANK_ACCOUNT_ID=" . urlencode($this->BANK_ACCOUNT_ID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["BANK_ACCOUNT_ID"]))
				$arKeys[] = ew_StripSlashes($_POST["BANK_ACCOUNT_ID"]);
			elseif (isset($_GET["BANK_ACCOUNT_ID"]))
				$arKeys[] = ew_StripSlashes($_GET["BANK_ACCOUNT_ID"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->BANK_ACCOUNT_ID->CurrentValue = $key;
