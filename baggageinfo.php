...
//
// Table class for baggage
//
class cbaggage extends cTable {
	var $BAGGAGE_ID;
	var $BOARDING_ID;
	var $PASSANGER_ID;
	var $WEIGHT;
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`baggage`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('BAGGAGE_ID', $rs))
				ew_AddFilter($where, ew_QuotedName('BAGGAGE_ID', $this->DBID) . '=' . ew_QuotedValue($rs['BAGGAGE_ID'], $this->BAGGAGE_ID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`BAGGAGE_ID` = @BAGGAGE_ID@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->BAGGAGE_ID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@BAGGAGE_ID@", ew_AdjustSql($this->BAGGAGE_ID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("baggageview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("baggageview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "baggageadd.php?" . $this->UrlParm($parm);
		else
			$url = "baggageadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("baggageedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("baggageadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("baggagedelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "BAGGAGE_ID:" . ew_VarToJson($this->BAGGAGE_ID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->BAGGAGE_ID->CurrentValue)) {
			$sUrl .= "BAGGAGE_ID=" . urlencode($this->BAGGAGE_ID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["BAGGAGE_ID"]))
				$arKeys[] = ew_StripSlashes($_POST["BAGGAGE_ID"]);
			elseif (isset($_GET["BAGGAGE_ID"]))
				$arKeys[] = ew_StripSlashes($_GET["BAGGAGE_ID"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->BAGGAGE_ID->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->BAGGAGE_ID->Exportable) $Doc->ExportCaption($this->BAGGAGE_ID);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->BAGGAGE_ID->Exportable) $Doc->ExportField($this->BAGGAGE_ID);
