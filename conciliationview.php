...
$conciliation_view = NULL; // Initialize page object first

class cconciliation_view extends cconciliation {
	...
	// Page object name
	var $PageObjName = 'conciliation_view';
	...
	function __construct() {
		...
		if (@$_GET["CONCILIATION_ID"] <> "") {
			$this->RecKey["CONCILIATION_ID"] = $_GET["CONCILIATION_ID"];
			$KeyUrl .= "&amp;CONCILIATION_ID=" . urlencode($this->RecKey["CONCILIATION_ID"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["CONCILIATION_ID"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["CONCILIATION_ID"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["CONCILIATION_ID"] <> "") {
				...
				$this->RecKey["CONCILIATION_ID"] = $this->CONCILIATION_ID->QueryStringValue;
			} elseif (@$_POST["CONCILIATION_ID"] <> "") {
				$this->CONCILIATION_ID->setFormValue($_POST["CONCILIATION_ID"]);
				$this->RecKey["CONCILIATION_ID"] = $this->CONCILIATION_ID->FormValue;
			} else {
				$sReturnUrl = "conciliationlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "conciliationlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "conciliationlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($conciliation_view)) $conciliation_view = new cconciliation_view();

// Page init
$conciliation_view->Page_Init();

// Page main
$conciliation_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$conciliation_view->Page_Render();
?>
...
var CurrentForm = fconciliationview = new ew_Form("fconciliationview", "view");

// Form_CustomValidate event
fconciliationview.Form_CustomValidate = 
...
fconciliationview.ValidateRequired = true;
<?php } else { ?>
fconciliationview.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $conciliation_view->ExportOptions->Render("body") ?>
<?php
	foreach ($conciliation_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if ($conciliation->Export == "") { ?>
...
<?php } ?>
<?php $conciliation_view->ShowPageHeader(); ?>
<?php
$conciliation_view->ShowMessage();
?>
<form name="fconciliationview" id="fconciliationview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($conciliation_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $conciliation_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="conciliation">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($conciliation->CONCILIATION_ID->Visible) { // CONCILIATION_ID ?>
	<tr id="r_CONCILIATION_ID">
		<td><span id="elh_conciliation_CONCILIATION_ID"><?php echo $conciliation->CONCILIATION_ID->FldCaption() ?></span></td>
...
<?php echo $conciliation->CONCILIATION_ID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($conciliation->UPLOAD_FILE_DETAIL_ID->Visible) { // UPLOAD_FILE_DETAIL_ID ?>
	<tr id="r_UPLOAD_FILE_DETAIL_ID">
		<td><span id="elh_conciliation_UPLOAD_FILE_DETAIL_ID"><?php echo $conciliation->UPLOAD_FILE_DETAIL_ID->FldCaption() ?></span></td>
...
<?php echo $conciliation->UPLOAD_FILE_DETAIL_ID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($conciliation->DATETIME->Visible) { // DATETIME ?>
	<tr id="r_DATETIME">
		<td><span id="elh_conciliation_DATETIME"><?php echo $conciliation->DATETIME->FldCaption() ?></span></td>
...
<?php echo $conciliation->DATETIME->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($conciliation->CARD_ID->Visible) { // CARD_ID ?>
	<tr id="r_CARD_ID">
		<td><span id="elh_conciliation_CARD_ID"><?php echo $conciliation->CARD_ID->FldCaption() ?></span></td>
...
<?php echo $conciliation->CARD_ID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($conciliation->TRANSFERENCE_ID->Visible) { // TRANSFERENCE_ID ?>
	<tr id="r_TRANSFERENCE_ID">
		<td><span id="elh_conciliation_TRANSFERENCE_ID"><?php echo $conciliation->TRANSFERENCE_ID->FldCaption() ?></span></td>
...
<?php echo $conciliation->TRANSFERENCE_ID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($conciliation->PAY_TYPE_ID->Visible) { // PAY_TYPE_ID ?>
	<tr id="r_PAY_TYPE_ID">
		<td><span id="elh_conciliation_PAY_TYPE_ID"><?php echo $conciliation->PAY_TYPE_ID->FldCaption() ?></span></td>
...
<?php echo $conciliation->PAY_TYPE_ID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($conciliation->Export == "") { ?>
<script type="text/javascript">
fconciliationview.Init();
</script>
<?php } ?>
<?php
$conciliation_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($conciliation->Export == "") { ?>
...
<?php
$conciliation_view->Page_Terminate();
?>
