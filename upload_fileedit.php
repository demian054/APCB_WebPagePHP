...
$upload_file_edit = NULL; // Initialize page object first

class cupload_file_edit extends cupload_file {
	...
	// Page object name
	var $PageObjName = 'upload_file_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->UPLOAD_FILE_ID->CurrentValue == "")
			$this->Page_Terminate("upload_filelist.php"); // Invalid key, return to list
	...
	function RenderRow() {
		...
		$this->DATETIME->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->PATH);
...
// Create page object
if (!isset($upload_file_edit)) $upload_file_edit = new cupload_file_edit();

// Page init
$upload_file_edit->Page_Init();

// Page main
$upload_file_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$upload_file_edit->Page_Render();
?>
...
var CurrentForm = fupload_fileedit = new ew_Form("fupload_fileedit", "edit");

// Validate form
fupload_fileedit.Validate = function() {
...
fupload_fileedit.Form_CustomValidate = 
...
fupload_fileedit.ValidateRequired = true;
<?php } else { ?>
fupload_fileedit.ValidateRequired = false; 
<?php } ?>
...
<?php $upload_file_edit->ShowPageHeader(); ?>
<?php
$upload_file_edit->ShowMessage();
?>
<form name="fupload_fileedit" id="fupload_fileedit" class="<?php echo $upload_file_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($upload_file_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $upload_file_edit->Token ?>">
<?php } ?>
...
<?php if ($upload_file->UPLOAD_FILE_ID->Visible) { // UPLOAD_FILE_ID ?>
	<div id="r_UPLOAD_FILE_ID" class="form-group">
		<label id="elh_upload_file_UPLOAD_FILE_ID" class="col-sm-2 control-label ewLabel"><?php echo $upload_file->UPLOAD_FILE_ID->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $upload_file->UPLOAD_FILE_ID->CellAttributes() ?>>
<span id="el_upload_file_UPLOAD_FILE_ID">
<span<?php echo $upload_file->UPLOAD_FILE_ID->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $upload_file->UPLOAD_FILE_ID->EditValue ?></p></span>
</span>
<input type="hidden" data-table="upload_file" data-field="x_UPLOAD_FILE_ID" name="x_UPLOAD_FILE_ID" id="x_UPLOAD_FILE_ID" value="<?php echo ew_HtmlEncode($upload_file->UPLOAD_FILE_ID->CurrentValue) ?>">
<?php echo $upload_file->UPLOAD_FILE_ID->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $upload_file_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fupload_fileedit.Init();
</script>
<?php
$upload_file_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$upload_file_edit->Page_Terminate();
?>
