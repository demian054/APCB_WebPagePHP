...
$upload_file_PATH_blobview = NULL; // Initialize page object first

class cupload_file_PATH_blobview extends cupload_file {
	...
	// Page object name
	var $PageObjName = 'upload_file_PATH_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('upload_file', 'x_PATH');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('PATH');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('PATH')));
				if (trim(strval($this->Recordset->fields('PATH'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('PATH') . "\"");
				}
...
// Create page object
if (!isset($upload_file_PATH_blobview)) $upload_file_PATH_blobview = new cupload_file_PATH_blobview();

// Page init
$upload_file_PATH_blobview->Page_Init();

// Page main
$upload_file_PATH_blobview->Page_Main();
?>
<?php
$upload_file_PATH_blobview->Page_Terminate();
?>
