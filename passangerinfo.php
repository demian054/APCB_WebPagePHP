...
//
class cpassanger extends cTable {
	var $PASSANGER_ID;
	var $CODE;
	var $FIRSTNAME;
	var $SECONDNAME;
	...
	function __construct() {
		...
		// PASSANGER_ID
		$this->PASSANGER_ID = new cField('passanger', 'passanger', 'x_PASSANGER_ID', 'PASSANGER_ID', '`PASSANGER_ID`', '`PASSANGER_ID`', 3, -1, FALSE, '`PASSANGER_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->PASSANGER_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['PASSANGER_ID'] = &$this->PASSANGER_ID;

		// CODE
		$this->CODE = new cField('passanger', 'passanger', 'x_CODE', 'CODE', '`CODE`', '`CODE`', 200, -1, FALSE, '`CODE`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['CODE'] = &$this->CODE;

		// FIRSTNAME
		$this->FIRSTNAME = new cField('passanger', 'passanger', 'x_FIRSTNAME', 'FIRSTNAME', '`FIRSTNAME`', '`FIRSTNAME`', 200, -1, FALSE, '`FIRSTNAME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['FIRSTNAME'] = &$this->FIRSTNAME;

		// SECONDNAME
		$this->SECONDNAME = new cField('passanger', 'passanger', 'x_SECONDNAME', 'SECONDNAME', '`SECONDNAME`', '`SECONDNAME`', 200, -1, FALSE, '`SECONDNAME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['SECONDNAME'] = &$this->SECONDNAME;

		// LASTNAME
		$this->LASTNAME = new cField('passanger', 'passanger', 'x_LASTNAME', 'LASTNAME', '`LASTNAME`', '`LASTNAME`', 200, -1, FALSE, '`LASTNAME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['LASTNAME'] = &$this->LASTNAME;

		// SURNAME
		$this->SURNAME = new cField('passanger', 'passanger', 'x_SURNAME', 'SURNAME', '`SURNAME`', '`SURNAME`', 200, -1, FALSE, '`SURNAME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['SURNAME'] = &$this->SURNAME;

		// MAIL
		$this->MAIL = new cField('passanger', 'passanger', 'x_MAIL', 'MAIL', '`MAIL`', '`MAIL`', 200, -1, FALSE, '`MAIL`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['MAIL'] = &$this->MAIL;

		// PASSANGER_TYPE_ID
		$this->PASSANGER_TYPE_ID = new cField('passanger', 'passanger', 'x_PASSANGER_TYPE_ID', 'PASSANGER_TYPE_ID', '`PASSANGER_TYPE_ID`', '`PASSANGER_TYPE_ID`', 3, -1, FALSE, '`PASSANGER_TYPE_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->PASSANGER_TYPE_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['PASSANGER_TYPE_ID'] = &$this->PASSANGER_TYPE_ID;

		// USER_ID
		$this->USER_ID = new cField('passanger', 'passanger', 'x_USER_ID', 'USER_ID', '`USER_ID`', '`USER_ID`', 3, -1, FALSE, '`USER_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`passanger`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('PASSANGER_ID', $rs))
				ew_AddFilter($where, ew_QuotedName('PASSANGER_ID', $this->DBID) . '=' . ew_QuotedValue($rs['PASSANGER_ID'], $this->PASSANGER_ID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`PASSANGER_ID` = @PASSANGER_ID@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->PASSANGER_ID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@PASSANGER_ID@", ew_AdjustSql($this->PASSANGER_ID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("passangerview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("passangerview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "passangeradd.php?" . $this->UrlParm($parm);
		else
			$url = "passangeradd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("passangeredit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("passangeradd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("passangerdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "PASSANGER_ID:" . ew_VarToJson($this->PASSANGER_ID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->PASSANGER_ID->CurrentValue)) {
			$sUrl .= "PASSANGER_ID=" . urlencode($this->PASSANGER_ID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["PASSANGER_ID"]))
				$arKeys[] = ew_StripSlashes($_POST["PASSANGER_ID"]);
			elseif (isset($_GET["PASSANGER_ID"]))
				$arKeys[] = ew_StripSlashes($_GET["PASSANGER_ID"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->PASSANGER_ID->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->PASSANGER_TYPE_ID->Exportable) $Doc->ExportCaption($this->PASSANGER_TYPE_ID);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->PASSANGER_TYPE_ID->Exportable) $Doc->ExportField($this->PASSANGER_TYPE_ID);
