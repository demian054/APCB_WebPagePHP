...
$passanger_edit = NULL; // Initialize page object first

class cpassanger_edit extends cpassanger {
	...
	// Page object name
	var $PageObjName = 'passanger_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->PASSANGER_ID->CurrentValue == "")
			$this->Page_Terminate("passangerlist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// PASSANGER_TYPE_ID
			$this->PASSANGER_TYPE_ID->SetDbValueDef($rsnew, $this->PASSANGER_TYPE_ID->CurrentValue, 0, $this->PASSANGER_TYPE_ID->ReadOnly);
...
// Create page object
if (!isset($passanger_edit)) $passanger_edit = new cpassanger_edit();

// Page init
$passanger_edit->Page_Init();

// Page main
$passanger_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$passanger_edit->Page_Render();
?>
...
var CurrentForm = fpassangeredit = new ew_Form("fpassangeredit", "edit");

// Validate form
fpassangeredit.Validate = function() {
...
fpassangeredit.Form_CustomValidate = 
...
fpassangeredit.ValidateRequired = true;
<?php } else { ?>
fpassangeredit.ValidateRequired = false; 
<?php } ?>
...
<?php $passanger_edit->ShowPageHeader(); ?>
<?php
$passanger_edit->ShowMessage();
?>
<form name="fpassangeredit" id="fpassangeredit" class="<?php echo $passanger_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($passanger_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $passanger_edit->Token ?>">
<?php } ?>
...
<?php if ($passanger->PASSANGER_ID->Visible) { // PASSANGER_ID ?>
	<div id="r_PASSANGER_ID" class="form-group">
		<label id="elh_passanger_PASSANGER_ID" class="col-sm-2 control-label ewLabel"><?php echo $passanger->PASSANGER_ID->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $passanger->PASSANGER_ID->CellAttributes() ?>>
<span id="el_passanger_PASSANGER_ID">
<span<?php echo $passanger->PASSANGER_ID->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $passanger->PASSANGER_ID->EditValue ?></p></span>
</span>
<input type="hidden" data-table="passanger" data-field="x_PASSANGER_ID" name="x_PASSANGER_ID" id="x_PASSANGER_ID" value="<?php echo ew_HtmlEncode($passanger->PASSANGER_ID->CurrentValue) ?>">
<?php echo $passanger->PASSANGER_ID->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $passanger_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
