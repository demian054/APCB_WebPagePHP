...
function CurrentProjectID() {
	...
	return "{B2F6402D-7C0A-4760-82AD-045088CEBA18}";
}
	...
	function ValidateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE) {
		...
		if (!$ValidateUser) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$ValidateUser = $CustomValidateUser || ew_ComparePassword($rs->fields('PASS'), $pwd, $encrypted);
					...
					if ($ValidateUser) {
						...
						$this->setCurrentUserName($rs->fields('CODE')); // Load user name
						$this->setSessionUserID($rs->fields('USER_ID')); // Load User ID
						if (is_null($rs->fields('USER_LEVEL_ID'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('USER_LEVEL_ID'))); // Load User Level
						}
						$this->SetUpUserLevel();
