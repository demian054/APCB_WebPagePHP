...
//
// Table class for air_port
//
class cair_port extends cTable {
	var $AIR_PORT_ID;
	var $CODE;
	var $DESCRIPTION;
	var $ACTIVE;
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`air_port`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('AIR_PORT_ID', $rs))
				ew_AddFilter($where, ew_QuotedName('AIR_PORT_ID', $this->DBID) . '=' . ew_QuotedValue($rs['AIR_PORT_ID'], $this->AIR_PORT_ID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`AIR_PORT_ID` = @AIR_PORT_ID@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->AIR_PORT_ID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@AIR_PORT_ID@", ew_AdjustSql($this->AIR_PORT_ID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("air_portview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("air_portview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "air_portadd.php?" . $this->UrlParm($parm);
		else
			$url = "air_portadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("air_portedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("air_portadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("air_portdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "AIR_PORT_ID:" . ew_VarToJson($this->AIR_PORT_ID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->AIR_PORT_ID->CurrentValue)) {
			$sUrl .= "AIR_PORT_ID=" . urlencode($this->AIR_PORT_ID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["AIR_PORT_ID"]))
				$arKeys[] = ew_StripSlashes($_POST["AIR_PORT_ID"]);
			elseif (isset($_GET["AIR_PORT_ID"]))
				$arKeys[] = ew_StripSlashes($_GET["AIR_PORT_ID"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->AIR_PORT_ID->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->AIR_PORT_ID->Exportable) $Doc->ExportCaption($this->AIR_PORT_ID);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->AIR_PORT_ID->Exportable) $Doc->ExportField($this->AIR_PORT_ID);
