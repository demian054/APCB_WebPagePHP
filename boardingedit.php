...
$boarding_edit = NULL; // Initialize page object first

class cboarding_edit extends cboarding {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->BOARDING_ID->CurrentValue == "")
			$this->Page_Terminate("boardinglist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// FLIGHT_ID
			$this->FLIGHT_ID->SetDbValueDef($rsnew, $this->FLIGHT_ID->CurrentValue, 0, $this->FLIGHT_ID->ReadOnly);

			// GATE
			$this->GATE->SetDbValueDef($rsnew, $this->GATE->CurrentValue, 0, $this->GATE->ReadOnly);

			// SEAT
			$this->SEAT->SetDbValueDef($rsnew, $this->SEAT->CurrentValue, "", $this->SEAT->ReadOnly);

			// DATETIME
			$this->DATETIME->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->DATETIME->CurrentValue, 7), ew_CurrentDate(), $this->DATETIME->ReadOnly);

			// RESERVATION_ID
			$this->RESERVATION_ID->SetDbValueDef($rsnew, $this->RESERVATION_ID->CurrentValue, 0, $this->RESERVATION_ID->ReadOnly);
...
// Create page object
if (!isset($boarding_edit)) $boarding_edit = new cboarding_edit();

// Page init
$boarding_edit->Page_Init();

// Page main
$boarding_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$boarding_edit->Page_Render();
?>
...
var CurrentForm = fboardingedit = new ew_Form("fboardingedit", "edit");

// Validate form
fboardingedit.Validate = function() {
...
fboardingedit.Form_CustomValidate = 
...
fboardingedit.ValidateRequired = true;
<?php } else { ?>
fboardingedit.ValidateRequired = false; 
<?php } ?>
...
<?php $boarding_edit->ShowPageHeader(); ?>
<?php
$boarding_edit->ShowMessage();
?>
<form name="fboardingedit" id="fboardingedit" class="<?php echo $boarding_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($boarding_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $boarding_edit->Token ?>">
<?php } ?>
...
<?php if ($boarding->BOARDING_ID->Visible) { // BOARDING_ID ?>
	<div id="r_BOARDING_ID" class="form-group">
		<label id="elh_boarding_BOARDING_ID" class="col-sm-2 control-label ewLabel"><?php echo $boarding->BOARDING_ID->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $boarding->BOARDING_ID->CellAttributes() ?>>
<span id="el_boarding_BOARDING_ID">
<span<?php echo $boarding->BOARDING_ID->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $boarding->BOARDING_ID->EditValue ?></p></span>
</span>
<input type="hidden" data-table="boarding" data-field="x_BOARDING_ID" name="x_BOARDING_ID" id="x_BOARDING_ID" value="<?php echo ew_HtmlEncode($boarding->BOARDING_ID->CurrentValue) ?>">
<?php echo $boarding->BOARDING_ID->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $boarding_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
