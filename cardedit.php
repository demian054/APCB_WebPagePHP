...
$card_edit = NULL; // Initialize page object first

class ccard_edit extends ccard {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->CARD_ID->CurrentValue == "")
			$this->Page_Terminate("cardlist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// NAME_IN_CARD
			$this->NAME_IN_CARD->SetDbValueDef($rsnew, $this->NAME_IN_CARD->CurrentValue, "", $this->NAME_IN_CARD->ReadOnly);

			// NUMBER
			$this->NUMBER->SetDbValueDef($rsnew, $this->NUMBER->CurrentValue, 0, $this->NUMBER->ReadOnly);

			// CARD_TYPE_ID
			$this->CARD_TYPE_ID->SetDbValueDef($rsnew, $this->CARD_TYPE_ID->CurrentValue, 0, $this->CARD_TYPE_ID->ReadOnly);

			// BANK_ID
			$this->BANK_ID->SetDbValueDef($rsnew, $this->BANK_ID->CurrentValue, 0, $this->BANK_ID->ReadOnly);

			// VALID_THRU_MONTH
			$this->VALID_THRU_MONTH->SetDbValueDef($rsnew, $this->VALID_THRU_MONTH->CurrentValue, 0, $this->VALID_THRU_MONTH->ReadOnly);

			// VALID_THRU_YEAR
			$this->VALID_THRU_YEAR->SetDbValueDef($rsnew, $this->VALID_THRU_YEAR->CurrentValue, 0, $this->VALID_THRU_YEAR->ReadOnly);
...
// Create page object
if (!isset($card_edit)) $card_edit = new ccard_edit();
...
var CurrentForm = fcardedit = new ew_Form("fcardedit", "edit");

// Validate form
fcardedit.Validate = function() {
...
fcardedit.ValidateRequired = true;
<?php } else { ?>
fcardedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcardedit.Lists["x_CARD_TYPE_ID"] = {"LinkField":"x_CARD_TYPE_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_DESCRIPTION","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcardedit.Lists["x_BANK_ID"] = {"LinkField":"x_BANK_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_DESCRIPTION","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $card_edit->ShowPageHeader(); ?>
<?php
$card_edit->ShowMessage();
?>
<form name="fcardedit" id="fcardedit" class="<?php echo $card_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($card_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $card_edit->Token ?>">
<?php } ?>
...
<input type="hidden" data-table="card" data-field="x_CARD_ID" name="x_CARD_ID" id="x_CARD_ID" value="<?php echo ew_HtmlEncode($card->CARD_ID->CurrentValue) ?>">
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $card_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
