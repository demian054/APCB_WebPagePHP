...
$upload_file_detail_edit = NULL; // Initialize page object first

class cupload_file_detail_edit extends cupload_file_detail {
	...
	// Page object name
	var $PageObjName = 'upload_file_detail_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->UPLOAD_FILE_DETAIL_ID->CurrentValue == "")
			$this->Page_Terminate("upload_file_detaillist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// UPLOAD_FILE_ID
			$this->UPLOAD_FILE_ID->SetDbValueDef($rsnew, $this->UPLOAD_FILE_ID->CurrentValue, 0, $this->UPLOAD_FILE_ID->ReadOnly);

			// UPLOAD_FILE_DETAIL_STATUS_ID
			$this->UPLOAD_FILE_DETAIL_STATUS_ID->SetDbValueDef($rsnew, $this->UPLOAD_FILE_DETAIL_STATUS_ID->CurrentValue, 0, $this->UPLOAD_FILE_DETAIL_STATUS_ID->ReadOnly);
...
// Create page object
if (!isset($upload_file_detail_edit)) $upload_file_detail_edit = new cupload_file_detail_edit();

// Page init
$upload_file_detail_edit->Page_Init();

// Page main
$upload_file_detail_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$upload_file_detail_edit->Page_Render();
?>
...
var CurrentForm = fupload_file_detailedit = new ew_Form("fupload_file_detailedit", "edit");

// Validate form
fupload_file_detailedit.Validate = function() {
...
fupload_file_detailedit.Form_CustomValidate = 
...
fupload_file_detailedit.ValidateRequired = true;
<?php } else { ?>
fupload_file_detailedit.ValidateRequired = false; 
<?php } ?>
...
<?php $upload_file_detail_edit->ShowPageHeader(); ?>
<?php
$upload_file_detail_edit->ShowMessage();
?>
<form name="fupload_file_detailedit" id="fupload_file_detailedit" class="<?php echo $upload_file_detail_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($upload_file_detail_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $upload_file_detail_edit->Token ?>">
<?php } ?>
...
<?php if ($upload_file_detail->UPLOAD_FILE_DETAIL_ID->Visible) { // UPLOAD_FILE_DETAIL_ID ?>
	<div id="r_UPLOAD_FILE_DETAIL_ID" class="form-group">
		<label id="elh_upload_file_detail_UPLOAD_FILE_DETAIL_ID" class="col-sm-2 control-label ewLabel"><?php echo $upload_file_detail->UPLOAD_FILE_DETAIL_ID->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $upload_file_detail->UPLOAD_FILE_DETAIL_ID->CellAttributes() ?>>
<span id="el_upload_file_detail_UPLOAD_FILE_DETAIL_ID">
<span<?php echo $upload_file_detail->UPLOAD_FILE_DETAIL_ID->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $upload_file_detail->UPLOAD_FILE_DETAIL_ID->EditValue ?></p></span>
</span>
<input type="hidden" data-table="upload_file_detail" data-field="x_UPLOAD_FILE_DETAIL_ID" name="x_UPLOAD_FILE_DETAIL_ID" id="x_UPLOAD_FILE_DETAIL_ID" value="<?php echo ew_HtmlEncode($upload_file_detail->UPLOAD_FILE_DETAIL_ID->CurrentValue) ?>">
<?php echo $upload_file_detail->UPLOAD_FILE_DETAIL_ID->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $upload_file_detail_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fupload_file_detailedit.Init();
</script>
<?php
$upload_file_detail_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$upload_file_detail_edit->Page_Terminate();
?>
