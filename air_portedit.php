...
$air_port_edit = NULL; // Initialize page object first

class cair_port_edit extends cair_port {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->AIR_PORT_ID->CurrentValue == "")
			$this->Page_Terminate("air_portlist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->AIR_PORT_ID->FldIsDetailKey)
			$this->AIR_PORT_ID->setFormValue($objForm->GetValue("x_AIR_PORT_ID"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->AIR_PORT_ID->CurrentValue = $this->AIR_PORT_ID->FormValue;
	...
	function RenderRow() {
		...
		$this->ACTIVE->ViewCustomAttributes = "";
			...
			// AIR_PORT_ID
			$this->AIR_PORT_ID->EditAttrs["class"] = "form-control";
			$this->AIR_PORT_ID->EditCustomAttributes = "";
			$this->AIR_PORT_ID->EditValue = $this->AIR_PORT_ID->CurrentValue;
			$this->AIR_PORT_ID->ViewCustomAttributes = "";
...
// Create page object
if (!isset($air_port_edit)) $air_port_edit = new cair_port_edit();

// Page init
$air_port_edit->Page_Init();

// Page main
$air_port_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$air_port_edit->Page_Render();
?>
...
var CurrentForm = fair_portedit = new ew_Form("fair_portedit", "edit");

// Validate form
fair_portedit.Validate = function() {
...
fair_portedit.Form_CustomValidate = 
...
fair_portedit.ValidateRequired = true;
<?php } else { ?>
fair_portedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fair_portedit.Lists["x_ACTIVE[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fair_portedit.Lists["x_ACTIVE[]"].Options = <?php echo json_encode($air_port->ACTIVE->Options()) ?>;
...
<?php $air_port_edit->ShowPageHeader(); ?>
<?php
$air_port_edit->ShowMessage();
?>
<form name="fair_portedit" id="fair_portedit" class="<?php echo $air_port_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($air_port_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $air_port_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="air_port">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<div>
<?php if ($air_port->AIR_PORT_ID->Visible) { // AIR_PORT_ID ?>
	<div id="r_AIR_PORT_ID" class="form-group">
		<label id="elh_air_port_AIR_PORT_ID" class="col-sm-2 control-label ewLabel"><?php echo $air_port->AIR_PORT_ID->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $air_port->AIR_PORT_ID->CellAttributes() ?>>
<span id="el_air_port_AIR_PORT_ID">
<span<?php echo $air_port->AIR_PORT_ID->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $air_port->AIR_PORT_ID->EditValue ?></p></span>
</span>
<input type="hidden" data-table="air_port" data-field="x_AIR_PORT_ID" name="x_AIR_PORT_ID" id="x_AIR_PORT_ID" value="<?php echo ew_HtmlEncode($air_port->AIR_PORT_ID->CurrentValue) ?>">
<?php echo $air_port->AIR_PORT_ID->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $air_port_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
