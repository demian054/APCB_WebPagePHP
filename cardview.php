...
$card_view = NULL; // Initialize page object first

class ccard_view extends ccard {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["CARD_ID"] <> "") {
			$this->RecKey["CARD_ID"] = $_GET["CARD_ID"];
			$KeyUrl .= "&amp;CARD_ID=" . urlencode($this->RecKey["CARD_ID"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["CARD_ID"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["CARD_ID"]);
		}
		...
		$this->CARD_ID->Visible = !$this->IsAdd() && !$this->IsCopy() && !$this->IsGridAdd();
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			} elseif (@$_POST["CARD_ID"] <> "") {
				$this->CARD_ID->setFormValue($_POST["CARD_ID"]);
				$this->RecKey["CARD_ID"] = $this->CARD_ID->FormValue;
			} else {
				$sReturnUrl = "cardlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "cardlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "cardlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_card\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_card',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fcardview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($card_view)) $card_view = new ccard_view();
...
var CurrentForm = fcardview = new ew_Form("fcardview", "view");

// Form_CustomValidate event
fcardview.Form_CustomValidate = 
...
fcardview.ValidateRequired = true;
<?php } else { ?>
fcardview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcardview.Lists["x_CARD_TYPE_ID"] = {"LinkField":"x_CARD_TYPE_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_DESCRIPTION","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcardview.Lists["x_BANK_ID"] = {"LinkField":"x_BANK_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_DESCRIPTION","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php } ?>
<?php $card_view->ExportOptions->Render("body") ?>
<?php
	foreach ($card_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if ($card->Export == "") { ?>
...
<?php } ?>
<?php $card_view->ShowPageHeader(); ?>
<?php
$card_view->ShowMessage();
?>
<form name="fcardview" id="fcardview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($card_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $card_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="card">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($card->CARD_ID->Visible) { // CARD_ID ?>
	<tr id="r_CARD_ID">
		<td><span id="elh_card_CARD_ID"><?php echo $card->CARD_ID->FldCaption() ?></span></td>
		<td data-name="CARD_ID"<?php echo $card->CARD_ID->CellAttributes() ?>>
<span id="el_card_CARD_ID">
<span<?php echo $card->CARD_ID->ViewAttributes() ?>>
<?php echo $card->CARD_ID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($card->NAME_IN_CARD->Visible) { // NAME_IN_CARD ?>
	<tr id="r_NAME_IN_CARD">
		<td><span id="elh_card_NAME_IN_CARD"><?php echo $card->NAME_IN_CARD->FldCaption() ?></span></td>
...
<?php echo $card->NAME_IN_CARD->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($card->NUMBER->Visible) { // NUMBER ?>
	<tr id="r_NUMBER">
		<td><span id="elh_card_NUMBER"><?php echo $card->NUMBER->FldCaption() ?></span></td>
		<td data-name="NUMBER"<?php echo $card->NUMBER->CellAttributes() ?>>
<span id="el_card_NUMBER">
<span<?php echo $card->NUMBER->ViewAttributes() ?>>
<?php echo $card->NUMBER->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($card->CARD_TYPE_ID->Visible) { // CARD_TYPE_ID ?>
	<tr id="r_CARD_TYPE_ID">
		<td><span id="elh_card_CARD_TYPE_ID"><?php echo $card->CARD_TYPE_ID->FldCaption() ?></span></td>
...
<?php echo $card->CARD_TYPE_ID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($card->BANK_ID->Visible) { // BANK_ID ?>
	<tr id="r_BANK_ID">
		<td><span id="elh_card_BANK_ID"><?php echo $card->BANK_ID->FldCaption() ?></span></td>
...
<?php echo $card->BANK_ID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($card->VALID_THRU_MONTH->Visible) { // VALID_THRU_MONTH ?>
	<tr id="r_VALID_THRU_MONTH">
		<td><span id="elh_card_VALID_THRU_MONTH"><?php echo $card->VALID_THRU_MONTH->FldCaption() ?></span></td>
...
<?php echo $card->VALID_THRU_MONTH->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($card->VALID_THRU_YEAR->Visible) { // VALID_THRU_YEAR ?>
	<tr id="r_VALID_THRU_YEAR">
		<td><span id="elh_card_VALID_THRU_YEAR"><?php echo $card->VALID_THRU_YEAR->FldCaption() ?></span></td>
...
<?php echo $card->VALID_THRU_YEAR->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
