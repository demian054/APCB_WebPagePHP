...
class cflight_add extends cflight {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["FLIGHT_ID"] != "") {
				...
				$this->setKey("FLIGHT_ID", $this->FLIGHT_ID->CurrentValue); // Set up key
			} else {
				$this->setKey("FLIGHT_ID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "flightview.php")
	...
	function LoadDefaultValues() {
		...
		$this->AIRPLANE_ID->CurrentValue = NULL;
		$this->AIRPLANE_ID->OldValue = $this->AIRPLANE_ID->CurrentValue;
		$this->AIR_PORT_ID_DEPARTURE->CurrentValue = NULL;
		$this->AIR_PORT_ID_DEPARTURE->OldValue = $this->AIR_PORT_ID_DEPARTURE->CurrentValue;
		$this->AIR_PORT_ID_ARRIVAL->CurrentValue = NULL;
		$this->AIR_PORT_ID_ARRIVAL->OldValue = $this->AIR_PORT_ID_ARRIVAL->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// AIRPLANE_ID
		$this->AIRPLANE_ID->SetDbValueDef($rsnew, $this->AIRPLANE_ID->CurrentValue, 0, FALSE);

		// AIR_PORT_ID_DEPARTURE
		$this->AIR_PORT_ID_DEPARTURE->SetDbValueDef($rsnew, $this->AIR_PORT_ID_DEPARTURE->CurrentValue, 0, FALSE);

		// AIR_PORT_ID_ARRIVAL
		$this->AIR_PORT_ID_ARRIVAL->SetDbValueDef($rsnew, $this->AIR_PORT_ID_ARRIVAL->CurrentValue, 0, FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->FLIGHT_ID->setDbValue($conn->Insert_ID());
				$rsnew['FLIGHT_ID'] = $this->FLIGHT_ID->DbValue;
			}
		} else {
...
var CurrentForm = fflightadd = new ew_Form("fflightadd", "add");
...
?>
<form name="fflightadd" id="fflightadd" class="<?php echo $flight_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($flight_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $flight_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $flight_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
